    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
	 ...
	 if( val->intval<0)  //add by alik :fix bug SSFZF-185
		{
		  val->intval=0;
		}
	 printk("read BAT_CAPACITY=%d  soc=%d ,checkpoint=%d ,sync=%d \n",val->intval,BMT_status.SOC,bat_volt_check_point,gSyncPercentage);
        break;        
...
/*                   remove by alik ,fix the bug:SWOWLJB-249 手机充电，99%~- 100%，这个阶段耗时1h30m，此段时间过长，请开发分析并作出修改 
                        if(bat_volt_check_point >= 100 )
                        {                    
...
*/                       gSyncPercentage=0;//fix bug :SWOWLJB-324
...
void mt_battery_notify_check(void)
{
    ...
    if(g_BN_TestMode == 0x0000)
    {
		...
		if( (BMT_status.ICharging > 1200) && //9091 标配1000mA的充电器.
...
void BAT_thread(void)
{
    ...
    if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else
        if(BMT_status.temperature >= 70/*60*/)  //modify by alik,when the temperature is up to 65,the app will power off the phone,otherwise when the temperature is up to 70 ,driver rst the phone at once.
        {
            #if defined(CONFIG_POWER_EXT)
	 ...
	 if((NONSTANDARD_CHARGER == CHR_Type_num) && (second_check ==0))
	{
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BAT_thread] 2nd CHR_Type_num=%d\r\n", CHR_Type_num);
	}
